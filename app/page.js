...
export default function Home() {
  ...
  return (
    <div className="fixed inset-0 flex flex-row overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      <aside className="w-72 bg-white/10 backdrop-blur-md border-r border-white/10 flex flex-col items-start p-5 overflow-y-auto">
        <h2 className="text-lg font-semibold mb-3 text-white/90">
          📋 Pengunjung Terakhir
        </h2>
        <ul className="w-full space-y-2">
          {recentVisitors.length === 0 ? (
            <p className="text-sm text-white/60">Belum ada data.</p>
          ) : (
            recentVisitors.map((v) => (
              <li
                key={v.id}
                className="flex justify-between items-center bg-white/10 rounded-lg p-2 px-3 text-sm hover:bg-white/20 transition-all duration-200"
              >
